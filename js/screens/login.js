...
// ============================================================================
import { APP_NAME, APP_EDITION, SOURCE_URL, buildEntryUrl } from "../config.js";
import { maskCpf, isValidCpf, normalizeCpf } from "../cpf.js";
import { icon, renderQrCode } from "../effects.js";
...
export function mount(root, { entry } = {}) {
  ...
  root.querySelector("#pineapple-slot").appendChild(icon("pineapple", { size: 72 }));
  ...
  wrap.innerHTML = `
    <div class="ticket">
      <div class="ticket__hero">
        <span id="pineapple-slot" class="pineapple-slot"></span>
      ...
      <details class="qr-tool">
        <summary>Sou da organização — gerar QR de entrada</summary>
        <div class="qr-tool__body">
          <label class="field">
            <span>Identificação do ponto de acesso (tenda, totem…)</span>
            <input id="qr-label" type="text" placeholder="ex.: TENDA-CENTRAL" maxlength="24">
          </label>
          <button type="button" id="qr-generate" class="btn btn--ghost btn--block">Gerar QR Code</button>
...
function wireQrTool(root) {
  root.querySelector("#qr-generate").addEventListener("click", async () => {
    const raw = root.querySelector("#qr-label").value.trim();
    const token = raw ? raw.toUpperCase().replace(/[^A-Z0-9-]/g, "-").slice(0, 24) : "CONVIDADO";
    const url = buildEntryUrl(token);
    const output = root.querySelector("#qr-output");
    const urlEl = root.querySelector("#qr-url");
    output.hidden = false;
    urlEl.textContent = url;
    const ok = await renderQrCode(root.querySelector("#qr-canvas"), url);
    if (!ok) urlEl.textContent += "  (não foi possível desenhar o QR agora — copie o link acima)";
  });
}
