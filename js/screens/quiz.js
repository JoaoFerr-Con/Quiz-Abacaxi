...
import { showMascotCelebration } from "../mascot.js";
...
export function mount(root) {
  ...
  root.querySelector("#streak-fire-slot").appendChild(icon("fire", { size: 26 }));
...
function renderQuestion(root) {
  ...
  area.innerHTML = "";
  area.appendChild(questionCard(root, round, question));

  startTimer(root, () => {
    // Blindado: se algo aqui lançar erro, o jogo não pode travar em silêncio —
    // melhor forçar a próxima pergunta do que deixar a tela estática.
    try {
      handleAnswer(root, -1, true);
    } catch (err) {
      console.error("[quiz] falha ao processar tempo esgotado:", err);
      advance(root);
    }
  });
}
...
function bump(el) {
  ...
  void el.offsetWidth; // força reflow pra reiniciar a animação mesmo se já estava rodando
  el.classList.add("is-bumping");
}

function renderStats(root) {
  const xpEl = root.querySelector("#stat-xp");
  const streakStatEl = root.querySelector("#stat-streak");
  const streakCountEl = root.querySelector("#streak-count");

  xpEl.textContent = `${state.xp} XP`;
  if (lastXp !== null && state.xp > lastXp) bump(xpEl);
  lastXp = state.xp;

  streakStatEl.classList.toggle("is-hot", state.streak >= RULES.streakStartsAt);
  streakCountEl.textContent = state.streak;
  if (lastStreak !== null && state.streak > lastStreak) bump(streakCountEl);
  lastStreak = state.streak;

  setStreakActive(root, state.streak >= RULES.streakStartsAt);
}

function questionCard(root, round, question) {
  ...
  question.options.forEach((text, i) => {
    ...
    btn.addEventListener("click", () => handleAnswer(root, i, false), { once: true });
    ...
    if (!isFreeform) {
      ...
      if (btn) {
        try {
          ...
          console.error("[quiz] confete falhou (efeito só visual, sem impacto no jogo):", err);
        }
      }
    }
    setState((s) => ({
...
function showFeedback(root, question, { isCorrect, isFreeform, earned, timedOut }) {
  ...
  const nextLabel = isLastQuestion() ? "Ver meu resultado" : "Próxima pergunta";
  ...
  feedback.innerHTML = `
    ...
    <button type="button" class="btn btn--primary btn--block" id="next-btn">${nextLabel} →</button>
  `;

  const nextBtn = feedback.querySelector("#next-btn");
  let advanced = false;
  const goAdvance = () => {
    if (advanced) return; // trava dupla: clique manual + auto-avanço não podem rodar os dois
    advanced = true;
    advance(root);
  };
  nextBtn.addEventListener("click", goAdvance);

  // Tempo esgotado: o jogo segue sozinho depois de um instante — não pode
  // depender da pessoa perceber/rolar até um botão pra não "travar" a tela.
  if (timedOut) {
    let secondsLeftToAdvance = 3;
    nextBtn.textContent = `${nextLabel} (${secondsLeftToAdvance}) →`;
    const autoTimer = setInterval(() => {
      secondsLeftToAdvance -= 1;
      if (secondsLeftToAdvance <= 0) {
    ...
    nextBtn.addEventListener("click", () => clearInterval(autoTimer), { once: true });
  }

  try {
    feedback.scrollIntoView({ behavior: "smooth", block: "center" });
  } catch {
    // scrollIntoView é só conforto visual — se falhar (navegador antigo,
    // ambiente sem layout), o jogo já segue sozinho pelo auto-avanço acima.
  }
}
...
function advance(root) {
  ...
  // Comemoração do mascote ao passar do limiar de acertos — só uma vez por partida.
  if (!state.mascotShown && state.correct >= RULES.mascotThreshold) {
    setState({ mascotShown: true });
    root.classList.add("theme-glow"); // fica "trocado de cor" pelo resto da partida
    showMascotCelebration(goNext);
    return;
  }
